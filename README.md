...
| `PLAYGUARD_EVAL_COMPACT` | `8000` | Max characters for eval output. `0` = off |
...
| `FIGMA_TEXT_COMPACT` | `8000` | Max characters for the Figma text response, applied whether or not the JSON optimizer ran (fallback for upstreams that return pre-simplified YAML/text, e.g. Framelink `figma-developer-mcp`). `0` = off |
...
Covers `collapseRuns`, `compactSnap` (including token budget boundary), and the full Figma optimizer pipeline (`optimizeFigmaResponse`) in `test/playguard.test.mjs`, plus `dead()` crash detection, `splitArgs()` quoting, the shared `ttlCache()` helper, and the `decideSnapshot()` cache/delta/hint decision logic in `test/playguard-core.test.mjs`.
...
playguard/
├── src/
│   └── index.ts                    All server logic (~690 lines)
